...
import {
  registerHot,
  resetHot,
  stubActiveQueryWithFirst,
  stubDom,
  stubHotInDocumentDom,
  stubHotRegisterWithActiveQuery,
  stubHotRegisterWithDefaultActiveQuery
} from '../helpers/basicHotHelper.js'
  ...
  describe('Active hot functions', function () {
    describe('Selecting cells', function () {
      ...
      it('returns current column of active hot after cell selection made', function () {
        ...
        sandbox.restore()
        stubHotRegisterWithActiveQuery(sandbox, stubActiveQueryWithFirst)
        hot.selectCell(0, 1)
        const result2 = hotFunctions.getCurrentColumnIndexOrMin()
        expect(result2).to.equal(1)
      })

      it('reselects [0,0] when no current cell can be reselected', function () {
        hot.loadData(data)
        const result = hotFunctions.getCurrentColumnIndexOrMin()
        expect(result).to.equal(0)
      })
