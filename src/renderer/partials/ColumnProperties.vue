        ...
        <div class="form-group-sm row container-fluid">
            <div class="propertyrow" v-for="(formprop, index) in formprops" :key="index">
                ...
                <template v-if="typeof formprop.type && formprop.type === 'dropdown'">
                    <select v-if="formprop.key==='type'" :value="getTypeProperty" v-model="typeProperty"
                            @input="setTypeProperty($event.target.value)" :id="formprop.key"
                    ...
                    <div id="format-container" v-if="formprop.key==='format'"
                        ...
                        <input v-if="formatValuesHasPattern" v-model="formatPropertyValue" type="text"
                               :class="{ 'form-control input-sm col-sm-9': true, 'validate-danger': errors.has('formatValue') }"
                               v-validate.initial="'formatPattern|required'" name="formatValue"/>
                        <div v-show="formatValuesHasPattern && errors.has('formatValue')"
                ...
                <div v-else-if="formprop.key === 'constraints'" id="constraints" class="col-sm-9">
                    <div class="input-group row" v-for="option in constraintValues" :key="option">
                        <input type="checkbox" :id="option" :checked="getConstraintCheck(option)"
                        ...
                        <template v-if="!isBooleanConstraint(option) && getConstraintCheck(option)">
                            <input type="text"
                                   ...
                                   v-validate.initial="constraintValidationRules(option)" :name="option"/>
                        </template>
                ...
                <textarea v-else-if="formprop.key === 'description'" rows="4" :value="getProperty(formprop.key)"
                          @input="setProperty(formprop.key, $event.target.value)" class="form-control label-sm col-sm-9"
                          :id="formprop.key"></textarea>
                <input v-else-if="formprop.key === 'name'" :disabled="formprop.isDisabled" :value="getNameProperty"
                       @input="setProperty(formprop.key, $event.target.value)" type="text"
                       class="form-control label-sm col-sm-9" :id="formprop.key"/>
                <template v-else-if="formprop.key === 'rdfType'">
                    <input :value="getProperty(formprop.key)" @input="setProperty(formprop.key, $event.target.value)"
                           type="text"
                           :class="{ 'form-control input-sm col-sm-9': true, 'validate-danger': errors.has(formprop.key) }"
                           v-validate="{url:true}" :id="formprop.key" :name="formprop.key"/>
                    <div v-show="formprop.key === 'rdfType' && errors.has(formprop.key)"
                         class="row help validate-danger">
                ...
                <template v-else-if="formprop.key === 'booleanType'">
                    <div class="extra-types input-group" v-show="typeProperty === 'boolean'">
                        ...
                        <input :value="getTrueValues()" @blur="setTrueValues($event.target.value)" type="text"
                               class="form-control label-sm col-sm-9" id="trueValues"/>
                    </div>
                    <div class="extra-types input-group" v-show="typeProperty === 'boolean'">
                        <label class="inline control-label col-sm-3" for="falseValues">False Values</label>
                        <input :value="getFalseValues()" @blur="setFalseValues($event.target.value)" type="text"
                               class="form-control label-sm col-sm-9" id="falseValues"/>
                    </div>
                </template>
                <template v-else-if="formprop.key === 'numberType'">
                    <div v-for="(extraType, eIndex) in formprop.types" :key="'number' + eIndex"
                         class="extra-types input-group" v-show="typeProperty === 'number'">
                        <label class="inline control-label col-sm-3" :for="formprop.key + extraType">{{getExtraPropertyLabel(extraType)}}</label>
                        <input v-if="extraType === 'bareNumber'" type="checkbox" :id="formprop.key + extraType"
                               :checked="getExtraType(extraType)" @click="setBareNumber($event.target)"/>
                        <input v-else :value="getExtraType(extraType)"
                               @input="setExtraType(extraType, $event.target.value)"
                               @blur="removeOnError(`${formprop.key}${extraType}`, extraType)" type="text"
                               :class="{ 'form-control label-sm col-sm-9': true,'validate-danger': errors.has(formprop.key + extraType) }"
                               v-validate="{max:1}" :id="formprop.key + extraType" :name="formprop.key + extraType"/>
                ...
                <template v-else-if="formprop.key === 'integerType'">
                    <div v-for="(extraType, eIndex) in formprop.types" :key="'integer' + eIndex"
                         class="extra-types input-group" v-show="typeProperty === 'integer'">
                        <template v-if="extraType === 'bareNumber'">
                            <label class="inline control-label col-sm-3" :for="formprop.key + extraType">{{getExtraPropertyLabel(extraType)}}</label>
                            <input type="checkbox" :id="formprop.key + extraType" :checked="getExtraType(extraType)"
                                   @click="setBareNumber($event.target)"/>
                        </template>
                    </div>
                </template>
                <input v-else :disabled="formprop.isDisabled" :value="getProperty(formprop.key)"
                       @input="setProperty(formprop.key, $event.target.value)" type="text"
                       class="form-control label-sm col-sm-9" :id="formprop.key"/>
            </div>
        </div>
