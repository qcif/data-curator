...
<div id="foreignKeyFields">
  <div v-for="(foreignKey,index) in hotForeignKeys"  class="foreign col-sm-12">
    <div class="inputs-container">
      ...
      <component v-show="isHeadersSelected" :key="getForeignComponentKey(index)" is="tableheaderkeys" :activeNames="currentForeignHeaders[index]" :getSelectedKeys="getSelectedForeignKeys(index)" :pushSelectedKeys="pushSelectedForeignKeys(index,currentLocalHotId)"/>
    </div>
...
export default {
  ...
  subscriptions() {
    return {
      ...
      // currentForeignHeaders: currentForeignHeaders$
    }
  },
  methods: {
  ...
  created: function() {
    ...
    let vueGetAllTableNamesHeaderNames = this.getAllTableNamesHeaderNames
    // this.$subscribeTo(selectedForeignTable$.startWith(['']).debounce(() => Observable.timer(100)), function(foreignTableNamesByIndex) {
    //   let currentForeignHeaders = []
    //   for (const [index, value] of foreignTableNamesByIndex.entries()) {
    //     currentForeignHeaders[index] = vueGetAllTableNamesHeaderNames(index)
    //   }
