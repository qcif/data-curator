...
import { allTablesAllColumnNames$, allTabsTitles$, fkPackagesButtonText$, loadingPackage$ } from '@/rxSubject.js'
    ...
    ipc.on('packageUrlLoaded', async function (event, index, hotId, url, descriptor) {
      ...
      if (dataPackage && dataPackage.valid) {
        ...
        if (dataPackage.resources.length > 0) {
          self.pushForeignKeysForeignTableForTable({
            hotId: hotId,
            index: index,
            resource: dataPackage.resources[0].name
          })
        }
