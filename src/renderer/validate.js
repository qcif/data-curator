...
var displayValidationMessages = function(validation) {
  ...
  var messages = _.flatten([
    _.map(validation.errors, function(d) {
      return _.extend({}, d, {msg_type: 'error'})
    }),
    _.map(validation.warnings, function(d) {
      return _.extend({}, d, {msg_type: 'warning'})
    })
  ])
  if (messages.length > 0) {
    $messagePanel.append(resultsTemplate({'validation': validation}))
    var elements = _.map(messages, function(message) {
      return $(messageTemplate(message)).data(message).addClass('message').addClass('validation-' + message.msg_type)
    })
    ...
    cells: function(row, col, prop) {
      if (d.row && d.col) {
        if (row === d.row - 1 && col === d.col - 1) {
          return {
            renderer: bgColorRenderer(colors[d.msg_type])
          }
        }
      } else {
        if (row === d.row - 1 || col === d.col - 1) {
